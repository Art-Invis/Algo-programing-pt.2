...
class MinCableLength(unittest.TestCase):
    def test_random_wells(self):
        csv_path = os.path.join(script_dir, '..', 'resources', 'communication_wells.csv')
        result = calculate_min_cable_length_from_csv(csv_path)
        self.assertEqual(result, 3000)

    def test_unconnected_wells(self):
        csv_path = os.path.join(script_dir, '..', 'resources', 'communication_unconnected_wells.csv')
        result = calculate_min_cable_length_from_csv(csv_path)
        self.assertEqual(result, -1)

    def test_empty_graph(self):
        csv_path = os.path.join(script_dir, '..', 'resources', 'communication_empty_graphs.csv')
        result = calculate_min_cable_length_from_csv(csv_path)
        self.assertEqual(result, -1)
